...
test("official CN entries warn that a human save is overwritten unless it is pinned", async () => {
  ...
  // Event and side stories never show the hint: the official-write rule leaves their human rows alone.
  assert.match(workspace, /const isStory = isEventStory \|\| isSideStory;/);
  assert.match(workspace, /!isStory && selectedEntry\.source === "cn" && \([\s\S]{0,400}SOURCE_LABELS\.pinned\}保存/);
