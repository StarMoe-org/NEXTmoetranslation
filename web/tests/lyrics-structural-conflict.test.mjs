...
test("LyricsEditor turns malformed synced Yjs state into a terminal read-only conflict", async () => {
  ...
  assert.match(editor, /const collaborationNeedsAttention = collaborationStructuralConflict \|\|/);
  assert.match(editor, /\{collaborationNeedsAttention && collaborationBanner\}[\s\S]*<details className="lyrics-technical-details">/);
