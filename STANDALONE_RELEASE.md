...
The workflow publishes an image digest, signature, and attestations. It does not change a runtime service. Production currently does not consume this image: Zeabur builds the Dockerfile's default target from source at `main`, so the deployed version is a commit SHA rather than an image digest (see `ROLLBACK_RUNBOOK.md`).
...
The server binary also embeds the accepted public-only Public Lyrics v3 runtime projection at `server/internal/publiclyricsbundle/public-v3.tar.gz` as the cold-start baseline for the public lyrics routes. Loading validates the closed inventory and rejects any nested, duplicate, non-regular, oversized, unexpected, or identity-mismatched member. It contains only `index.json` and `music_<id>.json`: no candidate manifest, receipt, producer database, recovery evidence, or authenticated/private input. Publication is the console button, not a rebuild of this file: every files-service rebuild projects the database first, database publications own their canonical `translation/lyrics/*` assets and all supported `v2/{locale}/translation/lyrics/*` mirrors, an explicit unpublish removes a song even when the bundle still contains it, and a bundle that fails to load degrades the projection to database-only instead of blocking the rebuild. The overlay does not migrate, replace, or write `/data/moesekai.db`.

The same `next-production` binary separately embeds the private editor seed at `server/internal/embeddedlyricsseed/editor-seed.tar.gz`. It is not mounted or served as a public asset. After migrations and before settings/account environment seeds, startup defers an empty catalog for the existing first-boot bootstrap, but requires every nonempty catalog to match the canonical seed exactly and atomically fills only missing lyrics ownership with native source-v3 documents, one legacy draft, and text-free availability states; the expected counts are compiled constants in `server/internal/embeddedlyricsseed/contract.go`. Existing legacy/source/recovery-availability ownership is preserved without modification; replay verifies the immutable batch/ledger and inserted ownership while creating zero new rows. A song replaced through `PUT /api/editor/v1/lyrics/document` has its `inserted` ledger items re-recorded as `preserved_existing` in the same transaction, so replay no longer compares it with the seed document. Catalog count/title/fingerprint drift, a different seed batch, or replay ownership loss fails startup. The seed excludes raw provider/recovery evidence, producer SQLite bytes, credentials, accounts, settings, ordinary translations, stories, reviews, and publications.
...
`contracts/public-lyrics/baseline.json` is the single authority for both embedded archives: it holds the public bundle's archive/batch/root SHA-256 values and member, catalog and detail counts, plus the editor seed's archive and producer-database SHA-256 values. No other file, and no document in this repository, restates those values.

- `scripts/build-public-lyrics-v3-bundle.py` and `scripts/build-embedded-lyrics-editor-seed.py` read their pins from the baseline instead of carrying a second copy.
- `scripts/verify-public-lyrics-baseline.py` hashes the committed archives and validates the bundle inventory, member count, and index/detail identity against the baseline.
- `scripts/verify-release.sh` loads the baseline, requires the Go constants to equal it (`BatchSHA256` and `RootSHA256` in `server/internal/publiclyricsbundle/bundle.go`, `ExpectedArchiveSHA256` in `server/internal/embeddedlyricsseed/bundle.go`), requires `README.md`, `PRODUCTION_CONTRACT.md`, and this document to reference the baseline path, and fails if any of them restates a pinned hash.

Changing an embedded artifact therefore means rebuilding it, updating the baseline, and rerunning `./scripts/verify-release.sh`; editing a hash in prose alone fails the gate.
