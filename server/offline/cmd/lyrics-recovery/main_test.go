...
func TestRecoveryCommandRejectsExtraBuildInputBeforeCatalogActivity(t *testing.T) {
	...
	writeCommandSourceFile(t, sourceRoot, "server/offline/cmd/lyrics-recovery/late-extra.s", []byte("TEXT ·late(SB),$0-0\nRET\n"))
...
func commandTestArguments(t *testing.T) []string {
	...
	const sourceFixturePath = "server/offline/cmd/lyrics-recovery/testdata/fixture.json"
	...
	writeCommandSourceFile(t, root, "server/offline/go.mod",
		[]byte("module example.invalid/recovery-command/offline\n\ngo 1.25\n\nreplace moesekai/server => ../\n"))
	writeCommandSourceFile(t, root, "server/offline/go.sum", []byte("example.invalid/module v1.0.0/go.mod h1:AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA=\n"))
	writeCommandSourceFile(t, root, "server/offline/cmd/lyrics-recovery/main.go", []byte("package main\n"))
	...
	fixtureManifest, err := json.Marshal(lyricsextractionplan.RecoveryFixtureManifestV2{
		...
		SelectionPolicy:   lyricsextractionplan.RecoverySourceSelectionPolicyV3,
	...
	writeCommandSourceFile(t, root, "server/offline/internal/lyricsextractionplan/recovery_source_fixtures_v2.json", fixtureManifest)
