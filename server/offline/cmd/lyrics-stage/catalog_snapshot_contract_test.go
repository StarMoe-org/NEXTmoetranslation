...
func TestMixedClassificationStagePublishesManifestReachableReceiptAndRealImportAcceptsPair(t *testing.T) {
	...
	results, _, err := offlineimport.ImportStagedLyricsManifestWithEvidenceReceiptAndCommitHook(
		context.Background(), store.New(importDatabase), publishedManifest, publishedReceipt, "offline-operator", nil,
	)
	...
	if _, _, err := offlineimport.ImportStagedLyricsManifestWithEvidenceReceiptAndCommitHook(
		context.Background(), store.New(importDatabase), publishedManifest, *report.EvidenceReceipt, "offline-operator", nil,
