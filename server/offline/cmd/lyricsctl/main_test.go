...
func TestDispatchSubcommandHelp(t *testing.T) {
	...
	for _, sub := range subcommands {
		t.Run(sub, func(t *testing.T) {
			...
			cli.FindCommand(sub).Handler = func(_ context.Context, args []string, _, _ io.Writer) error {
...
func TestDispatchForwardsDelegateOnlyFlagsVerbatim(t *testing.T) {
	...
	for sub, args := range cases {
		t.Run(sub, func(t *testing.T) {
			var got []string
			cli.FindCommand(sub).Handler = func(_ context.Context, forwarded []string, _, _ io.Writer) error {
...
func TestSubcommandsDispatchToRegisteredHandlers(t *testing.T) {
	...
	for name, args := range subcommands {
		cmdName := name
		expectedArgs := args
		cli.FindCommand(cmdName).Handler = func(ctx context.Context, passedArgs []string, stdout, stderr io.Writer) error {
