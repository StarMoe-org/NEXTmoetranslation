...
func setupStagedManifestImportStore(t *testing.T, songs []stagedImportSong) (*store.Store, *db.DB, lyricsstaging.Manifest, lyricsstaging.PrivateEvidenceReceipt) {
	...
	for index, song := range songs {
		...
		if provider == model.LyricsSourceProviderSekaipedia {
			listRaw, readErr := os.ReadFile("../../../internal/lyricssource/testdata/sekaipedia-list-335193.json")
			if readErr != nil {
				t.Fatal(readErr)
			}
...
func TestImportStagedLyricsManifestPreservesSourceTranslationInPrivateDraft(t *testing.T) {
	...
	results, _, err := ImportStagedLyricsManifestWithEvidenceReceiptAndCommitHook(context.Background(), s, manifest, receipt, "offline-operator", nil)
	if err != nil {
		t.Fatal(err)
	}
	...
	replayed, _, err := ImportStagedLyricsManifestWithEvidenceReceiptAndCommitHook(context.Background(), s, manifest, receipt, "offline-operator", nil)
...
func TestImportStagedLyricsManifestCreatesPrivateEditableDraftAndReplays(t *testing.T) {
	...
	replay, _, err := ImportStagedLyricsManifestWithEvidenceReceiptAndCommitHook(context.Background(), s, manifest, receipt, "offline-operator", nil)
...
func TestImportStagedLyricsManifestRejectsOmittedCrossedDuplicateAndOrphanEvidenceReceipts(t *testing.T) {
	t.Run("omitted successful evidence", func(t *testing.T) {
		...
		if _, _, err := ImportStagedLyricsManifestWithEvidenceReceiptAndCommitHook(context.Background(), s, manifest, omitted, "offline-operator", nil); err == nil || !strings.Contains(err.Error(), "candidate reference is unresolved") {
	...
	t.Run("crossed receipt", func(t *testing.T) {
		...
		if _, _, err := ImportStagedLyricsManifestWithEvidenceReceiptAndCommitHook(context.Background(), s, manifest, crossed, "offline-operator", nil); err == nil || !strings.Contains(err.Error(), "candidate reference is unresolved") {
	...
	t.Run("extra projected evidence remains an import orphan", func(t *testing.T) {
		...
		if _, _, err := ImportStagedLyricsManifestWithEvidenceReceiptAndCommitHook(context.Background(), s, manifest, orphaned, "offline-operator", nil); err == nil || !strings.Contains(err.Error(), "private evidence receipt contains orphan evidence") {
	...
	t.Run("duplicate evidence ID", func(t *testing.T) {
		...
		if _, _, err := ImportStagedLyricsManifestWithEvidenceReceiptAndCommitHook(context.Background(), s, manifest, duplicated, "offline-operator", nil); err == nil || !strings.Contains(err.Error(), "not uniquely ordered by evidence ID") {
...
func TestStagedLyricsSourceProvenanceSurvivesTransactionalContentBackupRestore(t *testing.T) {
	s, database, manifest, receipt := setupStagedManifestImportStore(t, []stagedImportSong{{musicID: 10, title: "合成試験曲", text: "初音歌う", sourceID: "miku"}})
	if _, _, err := ImportStagedLyricsManifestWithEvidenceReceiptAndCommitHook(context.Background(), s, manifest, receipt, "offline-operator", nil); err != nil {
		t.Fatal(err)
	}
	...
	if replay, _, err := ImportStagedLyricsManifestWithEvidenceReceiptAndCommitHook(context.Background(), s, manifest, receipt, "offline-operator", nil); err != nil || len(replay) != 1 || replay[0].Changed {
...
func TestImportStagedLyricsManifestPreservesEachFixedRevisionFetchTime(t *testing.T) {
	...
	results, _, err := ImportStagedLyricsManifestWithCommitHook(context.Background(), s, manifest, "offline-operator", nil)
	if err != nil {
		t.Fatal(err)
	}
...
func TestImportStagedLyricsManifestConflictsWithNonidenticalExistingDraft(t *testing.T) {
	...
	if _, _, err := ImportStagedLyricsManifestWithCommitHook(context.Background(), s, manifest, "offline-operator", nil); !errors.Is(err, ErrLyricsStagedManifestConflict) {
...
func TestImportStagedLyricsManifestRollsBackWholeBatch(t *testing.T) {
	...
	if _, _, err := ImportStagedLyricsManifestWithCommitHook(context.Background(), s, manifest, "offline-operator", nil); err == nil || !strings.Contains(err.Error(), "injected staged import failure") {
...
func TestImportStagedLyricsManifestRejectsCatalogDriftAndProjectsDeclaredExternalPerformer(t *testing.T) {
	t.Run("catalog fingerprint", func(t *testing.T) {
		...
		if _, _, err := ImportStagedLyricsManifestWithCommitHook(context.Background(), s, manifest, "offline-operator", nil); !errors.Is(err, ErrLyricsStagedManifestDrift) {
