...
- 公开文件的去抖重建现在最多等两个窗口：窗口仍是 `FILES_REBUILD_DEBOUNCE_MS`（默认 300000 ms，100–86400000），每次改动重新计时，但从开始等待算起两个窗口后一定开始重建，每分钟一轮的回填写入不会一直推迟所有公开文件的发布（`TestDebouncedWriteWaitsOneWindow`、`TestSteadyWritesStillPublishWithinTwoDebounceWindows`）。`/readyz` 在已请求的重建发布之前返回 503：回填首轮（首次部署 v39 时）或任何目录有变化的一轮结束时会请求去抖重建，所以 `/readyz` 首次返回 200 之后可能再回到 503，最长两个窗口，这不算部署失败（见 `ROLLBACK_RUNBOOK.md`）。搜索索引的去抖（`SEARCH_INDEX_DEBOUNCE_MS`，默认 1 小时）同样最多等两个窗口，回填期间持续的改动通知不会让搜索索引一直不重建（`TestSteadyTriggersStillRebuildWithinTwoDebounceWindows`）。
