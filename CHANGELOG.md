...
- 内容备份的 `translation-content/manifest.json` 升到 `schemaVersion` 2，新增第四个文件 `translation-content/side-stories.json`。早于 v39 的程序拒绝恢复它，目标库不变：回填之后 `side-stories.json` 超过旧程序给未知文件的 64 MiB 上限，S3 与 Git 恢复在解包时就报 `archive file translation-content/side-stories.json exceeds 67108864 bytes`，更小的备份才会走到 manifest 检查并报 `unsupported translation content schemaVersion 2`；本版本恢复 `schemaVersion` 1 或没有 manifest 的旧备份时，会清空四张 side-story 表。回滚步骤见 `ROLLBACK_RUNBOOK.md`。
...
- 新增 env `SIDE_STORY_BACKFILL_ENABLED`（默认 `true`）、`SIDE_STORY_BACKFILL_INTERVAL_MS`（默认 60000，1000–86400000）、`SIDE_STORY_BACKFILL_BATCH`（默认 30，1–500）、`SIDE_STORY_BACKFILL_REQUEST_DELAY_MS`（默认 1000，100–60000）。每次启动都在打开数据库之前校验，非法值会让启动失败，数据库不会先被迁移（`TestSideStoryBackfillConfigurationDefaultsAndIsStrict`、`TestInvalidSideStoryEnvFailsBeforeTheDatabaseIsOpened`）。运行时开关是新设置 `side_story_backfill.enabled`（未设置即为开，管理设置弹窗里可改），它与旧版 CN 自动同步的 `scheduler.enabled` 无关：生产上 `scheduler.enabled` 一直为 false，打开它会在上游版本变化时同步 CN 内容并对新活动剧情自动调用 AI，所以回填不再借用它。想在验证完成前关掉回填，只能在部署时设 `SIDE_STORY_BACKFILL_ENABLED=false`：首轮随服务启动立即运行，而 v38 的程序拒绝写入未知的设置键，没法事先把该设置改为 false。env 为 false 时回填循环根本不启动，没有目录，部署后冒烟里需要故事的几步都做不了；之后要开启只能改 env 并重启，管理设置覆盖不了 env（`TestSideStoryBackfillRunsOnlyWhileItsSettingAndEnvAllowIt`）。
- 新增 6 个上游设置：`upstream.en_masterdata_url`、`upstream.en_masterdata_fallback_url`、`upstream.jp_scripts_url`、`upstream.jp_scripts_fallback_url`、`upstream.cn_scripts_url`、`upstream.en_scripts_url`，分别由 `UPSTREAM_EN_MASTERDATA_URL`、`UPSTREAM_EN_MASTERDATA_FALLBACK_URL`、`UPSTREAM_JP_SCRIPTS_URL`、`UPSTREAM_JP_SCRIPTS_FALLBACK_URL`、`UPSTREAM_CN_SCRIPTS_URL`、`UPSTREAM_EN_SCRIPTS_URL` 首次写入。默认值和主机见 `PRODUCTION_CONTRACT.md`，生产环境按 `validateUpstreamURLSetting` 校验（`TestUpstreamURLSettingsFailClosedInProduction`、`TestSideStorySourcesDefaultToTheStartappMirrorsAndFollowOverrides`、`TestSeedConfigFromEnvSeedsTheSideStorySourceBases`）。这 6 个 env 每次启动、打开数据库之前还会用首次写入时的同一个校验函数（`config.ValidateSetting`）检查原始值：只含空白视为未设置，不安全的地址、控制字符（包括行尾的回车）或超过 8192 字节都会让启动失败，报错为 `side story source configuration: <变量名>: <原因>`（`TestInvalidSideStoryEnvFailsBeforeTheDatabaseIsOpened`）。`.env.example` 列出了这 6 个上游变量和上面 4 个回填变量的默认值。
- 公开文件的去抖重建现在最多等两个窗口：窗口仍是 `FILES_REBUILD_DEBOUNCE_MS`（默认 300000 ms，100–86400000），每次改动重新计时，但从开始等待算起两个窗口后一定开始重建，回填持续每轮写入时也不会一直推迟所有公开文件的发布（`TestDebouncedWriteWaitsOneWindow`、`TestSteadyWritesStillPublishWithinTwoDebounceWindows`）。`/readyz` 在已请求的重建发布之前返回 503：回填首轮（首次部署 v39 时）或任何目录有变化的一轮结束时会请求去抖重建，所以 `/readyz` 首次返回 200 之后可能再回到 503，最长两个窗口，这不算部署失败（见 `ROLLBACK_RUNBOOK.md`）。搜索索引的去抖（`SEARCH_INDEX_DEBOUNCE_MS`，默认 1 小时）同样最多等两个窗口，回填期间持续的改动通知不会让搜索索引一直不重建（`TestSteadyTriggersStillRebuildWithinTwoDebounceWindows`）。
- 磁盘：部署后 `/data` 会同时留着 `.pre-migration-v39.bak` 和 2026-09-24 那次的 `.pre-migration-v35.bak`，首次回填让数据库再增加约 0.2 GB，每次内容备份还要在 `/data/backup-work` 下临时写一份整库 `VACUUM INTO` 快照和约 0.6 GB 的 JSON。按演练时 0.44 GB 的 v38 库估算，稳态约 1.5 GB，备份峰值再加约 1.3 GB，计算方法见 `ROLLBACK_RUNBOOK.md`。
...
- `GET /api/editor/v1/stories` 按 `kind`、`locale`、`status` 列出故事及翻译进度，`GET /api/editor/v1/story/{kind}/{id}` 返回全部话和行（`TestSideStoryListFiltersByStatusAndValidatesQuery`、`TestSideStoryDetailValidatesIdentityAndLocale`、`TestListSideStoriesCountsLinesStatusAndPrimarySource`）。列表的 `status` 只按正文和说话人行判定：已抓到日文、但只有官方话标题有译文的故事列为 `untranslated`，`primarySource` 为 `""`，计数字段仍包含标题行。
- `PUT /api/editor/v1/story/{kind}/{id}/{episode}` 经 `strictContentMutation`，每行带 `expectedRevision`，整批全有或全无：未知行返回 `422 unknown_lines`，修订冲突返回 `409 revision_conflict` 并在 `current.conflicts` 里给出当前文本。有改动时，响应前重建该故事的公开文件并广播 `sidestory.updated`（`TestSideStoryUpdateWritesPublishesAndReportsConflicts`、`TestSideStoryUpdateIsAStrictContentMutation`、`TestSideStoryUpdatePublishesThePublicFileIncrementally`、`TestUpdateSideStoryLinesIsAllOrNothing`、`TestUpdateSideStoryLinesValidatesInput`）。只含空白的文本按空串存（`TestUpdateSideStoryLinesStoresWhitespaceOnlyTextAsAnEmptyLine`）。
- `GET /api/editor/v1/story/{kind}/{id}/{episode}/snapshot` 现抓日文脚本，与库里的 SHA-256 比对后，按活动剧情快照的格式导出，控制台的 TXT 导入器可以直接读取（`TestSideStorySnapshotSatisfiesTheConsoleImporter`、`TestSideStorySnapshotRejectsMissingOrChangedScripts`）。快照的 `scenario.scenarioId` 取剧本自身的 `ScenarioId`，所以 `ScenarioId` 只是标签的卡牌剧本也能用 TXT 导入（`TestSideStorySnapshotOfALabelledScriptSatisfiesTheConsoleImporter`）。
- `POST /api/editor/v1/story/{kind}/{id}/ai` 作为 producer 任务 `ai-side-story` 运行，只填空行，写成 `llm`，不覆盖列出目标之后被改过的行（`TestSideStoryAITranslatesThroughTheRunner`、`TestSideStoryAITranslationsOnlyFillLinesUnchangedSinceTheirTargets`、`TestSideStoryAIRunsOnlyOnDemandWithTheLocalePrompt`）。每批单独提交：中途某批失败时，之前已保存的批次立即发布并广播，`details` 末尾带 `translated lines saved before the failure: N`；一行都没写时不发布也不广播。LLM 服务商故障（重试后仍失败，如 HTTP 429 或回复无法解析）返回 `502 upstream_unavailable`，缺少 API key、provider 不受支持和取消仍是 `500 internal_error`（`TestSideStoryAIPublishesExactlyWhenItSavedLines`、`TestSideStoryAIMapsLLMFailuresToUpstreamUnavailable`）。判断依据是 translator 的类型化错误 `ErrLLMFailed` 与 `ErrLLMKeyMissing`，不再匹配错误文本，所以服务商回复里即使出现缺 key 的文字，也仍返回 502（`TestSideStoryAIClassifiesLLMFailuresByType`、`TestCallLLMWrapsTypedErrors`）。`en-US` 使用单独的英文 prompt，已有调用方的中文 prompt 逐字节不变（`TestExistingLLMCallersKeepTheChinesePromptByteForByte`）。
- `POST /api/editor/v1/story/{kind}/{id}/refresh` 立即重抓一个故事的日文、CN、EN 脚本并导入，不受回填的请求间隔限制（`TestSideStoryRefreshReturnsTheRunnerApplies`、`TestRefreshSideStoryFetchesEveryLocaleNow`）。某一话的日文抓取失败时，只有后台回填还会再抓这一话，才记进 `lastError` 并安排重试；否则只在响应里返回，已有的脚本、行和状态不变（`TestApplySideStoryJPFailureLeavesAStoredScriptTheBackfillWillNotFetchAgain`）。`GET` / `POST /api/editor/v1/stories/sync` 读取回填状态或立即唤醒一轮（`TestSideStoryBackfillRoutes`）。
- 新 SSE 事件 `sidestory.updated` 与 `sidestory.sync`（`TestSideStoryEventNamesMatchTheConsoleVocabulary`）。

### 后台回填

- `translator.SideStoryBackfill` 在服务进程里按轮运行：到期时从 masterdata 重建目录，再以请求间隔抓取到期的话，每 5 话在编辑准入和共享内容锁下写入一次。请求间隔跨轮保持，只约束后台回填（`TestSideStoryBackfillKeepsTheRequestDelayAcrossRounds`）。它不是 producer，不调用 LLM，producer 运行时整轮推迟，最迟 5 秒后重试；在发出任何请求之前就推迟的一轮不覆盖上一轮的时间、统计和错误，回填面板不会显示「0 话」和英文的推迟提示（`TestSideStoryBackfillKeepsTheLastRoundWhileDeferredToAProducer`、`TestSideStoryBackfillReportsARoundDeferredAfterItsCatalogRequests`、`TestSideStoryBackfillRoundImportsOfficialChineseAndEnglish`、`TestSideStoryBackfillDefersWritesWhileAProducerRuns`、`TestSideStoryBackfillRunsOnlyWhileItsSettingAndEnvAllowIt`、`TestServicesConnectTheSideStoryRunnerAndStopItOnShutdown`）。一轮结束后再等 `SIDE_STORY_BACKFILL_INTERVAL_MS` 才开始下一轮：默认值下 30 话的抓取约 145 秒（每次请求约 1.7 秒），加上 60 秒间隔约 205 秒一轮，约 5.6k 话的首次回填需要 10–11 小时，约 1,500 次上游请求/小时（`contracts/editor-api/README.md` 8.8、`ROLLBACK_RUNBOOK.md`）。
- 官方 CN/EN 文本按 TalkData 下标与日文配对。条数不符时，该语言标为 `mismatch`，一行都不写。脚本按资源路径识别，脚本内的 ScenarioId 字段只是标签，不做比较（真实脚本里有 `016048_rui01 のコピー`、`★4冬弥・泉_前半` 这样的值）。超过一半的含假名正文与日文相同（镜像站还在提供新上架脚本的日文占位）时同样一行都不写，但该语言保持 `pending`，`lastError` 记 `zh-CN: official script repeats the Japanese text` 或 `en-US: official script repeats the Japanese text`，与 404 一样 24 小时后重试、不计入 attempts。官方文本覆盖文本不同的 `official` 行，并接管所有 `llm` 行（文本相同的 `llm` 行也改为 `official`、`sync`，revision 加 1），从不改动 `human` 行（`TestApplySideStoryOfficialImportTakesOverIdenticalLLMRows`、`TestApplySideStoryOfficialImportPairsByTalkDataIndex`、`TestApplySideStoryOfficialImportRejectsMismatchedScriptsAndRetriesAMirroredOne`、`TestSideStoryBackfillKeepsHumanRowsAndRefreshesTheCatalogOnDataVersionAndAge`）。
- 已列出资源路径的 CN/EN 脚本返回 404（镜像还没同步）时，该语言保持 `pending`，`lastError` 记 `zh-CN: not found` 或 `en-US: not found`，24 小时后重试，不计入 attempts；另一种语言同时有暂时性错误时，按那边较早的退避时间重试。已导入过的日文脚本变化时，这次没有抓取的语言回到 `pending`，下一轮就抓，另一种语言这次 404 也不必等 24 小时；另一种语言的暂时性错误仍按它的退避时间。目录里 CN/EN 资源路径变化使该语言回到 `pending`，或 JP 资源路径变化时，同时清零 attempts 和下次重试时间，新路径下一轮就抓，不等旧路径的 404 重试；`lastError` 也随之清掉，只保留仍处于 `mismatch` 或 `error` 的语言那一段，因为这些语言不会再抓（`TestSyncSideStoryCatalogKeepsTheReasonOfALocaleLeftInMismatch`）。CN/EN 资源路径变空、该语言变为 `absent` 时，`lastError` 删去该语言那一段；这一话仍在等其他重试时，日文段、另一种语言的段和重试时间保留，否则同样清零 attempts 和下次重试时间（`TestSyncSideStoryCatalogRequeuesChangedOfficialPathsAndMarksRemovedOnesAbsent`、`TestSyncSideStoryCatalogKeepsTheJPReasonOfAVanishedPathOnlyWhileQueued`）。日文脚本没变时，已 `imported` 的语言后来遇到 404 或镜像返回日文仍保持 `imported`，不安排重试，也不返回、不记录任何消息，回填面板不把它算作「错误」或「待重试」（`TestApplySideStoryKeepsAnImportAgainstALaterMissingOrMirroredFetch`）；处于 `mismatch` 或 `error` 的语言，在另一种语言重试或日文失败时保留自己那段 `lastError`（`TestApplySideStoryKeepsTheReasonOfALocaleLeftInErrorWithoutAFetch`）。`absent` 只表示该服务器的 masterdata 没有这一话的资源路径（`TestApplySideStoryOfficial404StaysPendingAndRetriesADayLater`、`TestSideStoryBackfillRetriesAMirrorServingJapaneseAndA404ADayLater`、`TestApplySideStoryRequeueAndANewAssetPathOutrankAnother404`、`TestSideStoryProgressCountsStatesPerKind`）。
...
- 内容备份恢复成功后，`afterContentRestore` 先退役协作会话，再调用 `TriggerSideStoryBackfill(true)`，回填开启时不必等 6 小时就重建目录。恢复期间仍持有 producer 门禁，被唤醒的一轮先推迟，之后最迟每 5 秒重试一次，门禁释放后就刷新；回填关闭期间请求的刷新保留到重新开启后的第一轮（`TestAfterContentRestoreRefreshesTheSideStoryCatalog`、`TestSideStoryBackfillKeepsACatalogRequestMadeWhileDisabled`）。
- 卡牌和区域两个目录分别记录刷新时间：哪个失败就只有它 10 分钟后再试，另一个不重抓；手动「刷新目录」失败时也一样，即使距它上次成功刷新不到 6 小时（`TestSideStoryBackfillRetriesAKindWhoseRequestedRefreshFailed`）；`catalogRefreshedAt` 取两者中较早的一次，两个都成功之前为空（`TestSideStoryBackfillRetriesOnlyTheCatalogKindThatFailed`）。masterdata 文件在所有源上都遇到暂时性错误时隔 500 ms 再试一次，与现有 masterdata 路径一致（`TestSideStoryMasterdataRetriesATransientFailureOnce`）。`upstream.state.last_data_version` 变化后的下一轮也会刷新目录。这个值由 upstream watcher 的轮询写入（需要 `scheduler.enabled`），也由管理员在管理设置里手动「立即检查」或「强制同步」上游时写入：还没有记录时写入基线，有新版本时由随之执行的 CN 同步写入；之后成功的手动 CN 同步也会写入检查发现的新版本。生产关闭 `scheduler.enabled`，只有这类手动操作会触发它，平时靠 6 小时周期或「刷新目录」。
- 每轮释放写入锁之后逐个重建本轮写过剧集的故事的公开文件，只有目录变化或某个故事发布失败时才请求去抖的全量重建（`TestSideStoryBackfillPublishesEpisodeWritesPerStoryAndRebuildsOnlyForTheCatalog`）。后台的日文脚本变化删掉人工译文时，记一行日志 `[side-story] <kind> <id> episode <key>: changed JP script deleted <N> human line translation(s)`（`TestSideStoryBackfillLogsHumanLinesDeletedByAJPScriptChange`）。
...
- 同一个公开文件的两次按故事发布（编辑器 PUT、refresh、AI 与回填轮次）依次执行：从读两种语言到写出文件都持有 filesvc 的互斥锁，先读库的一次不会用旧字节覆盖刚提交的编辑，区域对话同组的其他故事也一样（`TestSideStoryPublicationDoesNotReplaceALaterOne`）。
...
- `side-stories.json` 的单文件上限为 512 MiB，是两种语言全部译完时估计的约 218 MiB（±15%）的两倍多，因为一个超限文件会让两个目标的整次备份失败；记录总数上限从 1,000,000 提到 2,000,000；其余上限不变（`TestSideStoriesContentFileHasItsOwnSizeLimit`、`TestTranslationContentRecordLimitLeavesRoomForProductionScaleSideStories`）。
...
- `gachaInfo` 不再导入官方 CN：国服 `gachaInformation` 是国服自己的公告（国服日期、周年序号、规则），不是日文的译文。`extractGachaInfo` 只按原样登记日服 `summary`、`bubbleText`、`description` 键，新键待译，已有译文不变，由 AI 按需翻译或由编辑与 agent 填写（`TestExtractGachaInfoRegistersExactJPKeysWithoutCN`）。生产的 `gachaInfo.json` 目前为 `{}`，没有已存的配对需要清理；这些键在下一次 CN 同步（设置里的「数据更新（CN 同步）」或管理设置的「强制同步」）时登记。
- `collectPair` 不再让后面缺国服文本的记录把同一日文已配对的官方中文清空，例如日服独有的复刻卡池与国服已有的卡池同名时；按 2026-09-25 的真实 `gachas.json`，`gacha` 分类的提取结果因此多保留 5 个卡池名的国服译名（如「初心者応援メモリアルセレクトガチャ」）（`TestExtractGachaKeepsCNNameSharedWithLaterJPOnlyGacha`、`TestCollectPairKeepsCNOverLaterRecordWithoutCN`）。
...
- 控制台细节：报错显示服务端 `details` 里的具体原因（如 AI 批次失败的 HTTP 429），`upstream_unavailable` 显示为「上游服务暂时不可用，请稍后重试」，AI 路由的 LLM 故障不再被说成上游剧本无法获取，`already_running`、`draining`、`internal_error` 有中文说明；`mismatch` 显示为「剧本结构不一致」；重抓结果和工具栏只按当前语言的状态区分「有错误」与「待重试」，404 和镜像仍返回日文不再算失败；TXT 导入把与不含假名的日文相同的文本（一歌、奏这类中日同形的名字）当作译文，与服务端官方导入的规则一致；在从未保存过的空行上「保存并下一条」只跳到下一条，不再写入永久的人工空行；来源修改和 TXT 批量遇到 409 时，对当前选中的行显示「采用服务器版本」；TXT 核对途中切换剧情、语言或话数不会让导入按钮一直停在「正在核对 TXT…」；1.5 秒内切换语言不会被旧语言的列表覆盖（`web/tests/side-story-console.test.mjs`、`web/tests/side-story-txt-import.test.mjs`、`web/tests/side-story-editor.test.mjs`）。管理设置新增「卡牌剧情/区域对话后台回填」开关。
- 控制台修复：在从未保存过的剧情行上改来源不再写入人工空行，该行的来源下拉框锁定；已保存的行输入纯空白时按空串保存；冲突落在没有草稿的选中行时，输入框换成服务器译文，横幅只提供「知道了」和「重新载入本篇」；手机宽度下远端修改横幅的按钮换到说明文字下一行；结果不确定的来源修改失败后，重新校对会保留其他行的未保存草稿（`web/tests/side-story-editor.test.mjs`）。收到 `sidestory.sync`、且打开的剧情出现只有回填才会造成的变化（标题、话数、已获取话数或行数变化，或官方译文条数增加）时，没有草稿就自动重新载入本篇，有草稿则显示远端修改横幅，草稿保存、采用远端版本或放弃后再重载；重载后仍选中原来那一行，等草稿保存后才重载时也一样；自己的保存与 sync 合并成一次列表载入时，不会被当成回填的改动（`web/tests/side-story-realtime-sync.test.mjs`、`web/tests/side-story-realtime-reload.test.mjs`）。工具栏新增「重新载入本篇」，重载后仍选中原来那一行（`web/tests/side-story-reload-selection.test.mjs`）；剧本还没载入或没获取时不再显示「已全部翻译」，`ja-JP` 视图显示各话条数；回填面板的导入状态沿用工具栏的名称，展开时回填正在运行或已到下一轮时间就每 15 秒刷新一次进度，这个轮询失败时不弹提示、保留上次的进度，展开时的首次载入和「刷新进度」按钮失败时仍会提示，它们还在进行时轮询先等它们结束，不会盖掉它们的失败提示；触发回填时如果正在运行，提示本轮结束后续跑一轮（`web/tests/side-story-toolbar-status.test.mjs`、`web/tests/side-story-toolbar-reload.test.mjs`、`web/tests/side-story-panel-labels.test.mjs`、`web/tests/side-story-panel-poll.test.mjs`、`web/tests/side-story-catalog-sync.test.mjs`）。AI 翻译等 producer 任务没跑完就失败时，顶部进度条在任务结束时消失，不再一直停在失败前的进度（`web/tests/console-progress-line.test.mjs`）。区域对话搜索也匹配侧栏分组名，搜索框的提示也写明了分组名；打开卡牌剧情或区域对话载入失败时，报错显示中文说明和 details（如「剧情不存在」），不再只显示 `not_found` 这类错误码（`web/tests/side-story-entries-errors.test.mjs`）；`script_changed` 提示请管理员先重新获取剧本，`already_running` 说明同步、AI 翻译或备份恢复任一任务都会挡住操作（`web/tests/side-story-console.test.mjs`）。TXT 导入（活动剧情与卡牌/区域剧情）只在 `zh-CN` 预览时把对白标点转成中文，`en-US` 保留原标点（`web/tests/event-txt-import.test.mjs`、`web/tests/side-story-txt-import.test.mjs`）。
- 上线前复审修复：「保存并下一条」按 key 从最新的条目里取下一行，保存期间协作者对下一行的修改会显示在输入框里，不会被再次保存改回旧译文（活动剧情同样适用）；按 README 8.3 交还给 AI、文本为空且来源不是人工的行，以空输入「保存并下一条」或在未保存提示中「先保存」都跳过写入，该行仍留给 AI 和官方导入，清空一条有文本的 AI 行则照常存成空的人工行，通过来源下拉框显式改来源也照常写入（`web/tests/side-story-editor.test.mjs`）。卡牌剧情与区域对话的列表刷新从第一次请求算起最迟 1.5 秒载入一次，agent 连续保存时侧栏、回填后的自动重载和回填面板进度也按时更新；刷新窗口到点时同类型的列表载入还在进行，就等它结束（采用或失败）后立即再载入一次，慢速网络下连续保存时列表照样会被采用；`sidestory.sync` 的重载判断只交给它登记之后开始、结果被采用的那次列表载入，载入被取代、失败或进行中又收到一次 sync 时，改动都留给下一次载入比对；卡牌剧情搜索接受侧栏显示的 `#1473` 写法（`web/tests/side-story-catalog-refresh.test.mjs`）。回填面板展开后首次载入进度失败时，15 秒轮询静默重试到拿到状态，面板不会一直停在「正在载入…」（`web/tests/side-story-panel-poll.test.mjs`）。有草稿时收到回填、之后按 Esc 关闭当前条目并选择放弃或保存，许诺的自动重新载入照常进行（`web/tests/side-story-realtime-reload.test.mjs`）。服务器不可达时，提示显示「<操作>失败：无法连接服务器」，不显示浏览器的英文报错（Failed to fetch / Load failed / NetworkError）（`web/tests/side-story-console.test.mjs`）。TXT 导入（活动剧情与卡牌/区域剧情）按标点转换前的 TXT 原文、逐行去掉首尾空白后判断是否仍是日文，保留 `…`、ASCII 标点或行尾全角空格的日文行不会被当作人工译文写入；zh-CN 保留「……」，不改写为「......」；正文换行数与日文不一致的行仍可勾选但不默认勾选，说明列写出两边的换行数；活动剧情预览仍把与日文相同的文本当作未翻译，因为这类行在 zh-CN 保存时常因缺少旧表记录返回 404（`web/tests/side-story-txt-import.test.mjs`、`web/tests/event-txt-import.test.mjs`）。
...
- 新分类 `gachaInfo` 排在 `gacha` 之后，字段为 `summary`、`bubbleText`、`description`，由 `extractGachaInfo` 从 `gachas.json` 的 `gachaInformation` 按 gacha id 配对 JP 与 CN（Phase S 起不再导入 CN，见 Phase S 一节的「gachaInfo」）。键保留原文的换行和首尾空白，不做 trim。CN 同步步骤表 `cnSyncSteps` 已加入该分类，公开文件 `translation/gachaInfo.json`、`gachaInfo.full.json` 及 `v2` 镜像自动生成（`TestExtractGachaInfoRegistersExactJPKeysWithoutCN`、`TestCNSyncStepsCoverEverySupportedCategory`、`TestUpdateEntryAcceptsLongMultilineGachaInfoDescription`）。
...
- 恢复后公开文件包含恢复出的活动剧情。此前恢复直接改写数据库，没有清空活动剧情列表缓存，恢复后的重建仍按恢复前的列表生成：新实例恢复后所有 `translation/eventStory/event_<id>.json` 都是 404，直到重启或下一次活动剧情写入。恢复现在在释放内容锁之前清空该缓存（`TestRestoreRefreshesCachedEventStorySummaries`）。
