...
- 卡牌和区域两个目录分别记录刷新时间：哪个失败就只有它 10 分钟后再试，另一个不重抓；`catalogRefreshedAt` 取两者中较早的一次，两个都成功之前为空（`TestSideStoryBackfillRetriesOnlyTheCatalogKindThatFailed`）。masterdata 文件在所有源上都遇到暂时性错误时隔 500 ms 再试一次，与现有 masterdata 路径一致（`TestSideStoryMasterdataRetriesATransientFailureOnce`）。按上游数据版本刷新目录要靠 upstream watcher，而它只在 `scheduler.enabled` 开启时运行，所以生产上只按 6 小时周期或手动「刷新目录」刷新。
...
- 控制台修复：在从未保存过的剧情行上改来源不再写入人工空行，该行的来源下拉框锁定；已保存的行输入纯空白时按空串保存；冲突落在没有草稿的选中行时，输入框换成服务器译文，横幅只提供「知道了」和「重新载入本篇」；结果不确定的来源修改失败后，重新校对会保留其他行的未保存草稿（`web/tests/side-story-editor.test.mjs`）。收到 `sidestory.sync` 且打开的剧情有变化时，没有草稿就自动重新载入本篇，有草稿则显示远端修改横幅，草稿保存、采用远端版本或放弃后再重载（`web/tests/side-story-realtime-sync.test.mjs`）。工具栏新增「重新载入本篇」；剧本还没载入或没获取时不再显示「已全部翻译」，`ja-JP` 视图显示各话条数；回填面板的导入状态沿用工具栏的名称，展开时回填正在运行或已到下一轮时间就每 15 秒刷新一次进度；触发回填时如果正在运行，提示本轮结束后续跑一轮（`web/tests/side-story-toolbar-status.test.mjs`、`web/tests/side-story-toolbar-reload.test.mjs`、`web/tests/side-story-panel-labels.test.mjs`、`web/tests/side-story-panel-poll.test.mjs`、`web/tests/side-story-catalog-sync.test.mjs`）。区域对话搜索也匹配侧栏分组名；`script_changed` 提示请管理员先重新获取剧本，`already_running` 说明同步、AI 翻译或备份恢复任一任务都会挡住操作（`web/tests/side-story-console.test.mjs`）。TXT 导入（活动剧情与卡牌/区域剧情）只在 `zh-CN` 预览时把对白标点转成中文，`en-US` 保留原标点（`web/tests/event-txt-import.test.mjs`、`web/tests/side-story-txt-import.test.mjs`）。
